...
// 檢查必要欄位是否存在
if (empty($block_id) || empty($block_start_date) || empty($block_end_date) || empty($seat_id) || empty($reason)) {
...
try {
    ...
    // 更新資料到 block_time 表
    $sql = "UPDATE block_time 
            SET block_start_date = :block_start_date, 
    ...
    if ($stmt->execute()) {
        ...
        echo json_encode(['success' => false, 'message' => '儲存修改失敗，請稍後再試']);
    }
} catch (Exception $e) {
    // 錯誤處理
